...
function HomePage({
  ...
  const difficultySections = difficultyConfigs
    .map((section): DifficultySection => ({
      ...section,
      scenarios: scenariosWithIndex.filter(({ scenario }) => scenario.difficulty === section.difficulty),
    }))
                ...
                return (
                  <article className="home-page__scenario" key={scenario.id}>
                    ...
                    <div className="home-page__scenario-main">
                      <h2>{formatScenarioTitle(scenario.title, originalIndex, t.common.caseLabel)}</h2>
                      <button
